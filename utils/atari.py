...
if __name__ == '__main__':
    env = Atari(name='pong',size=[64, 64], repeat=4, sticky=False, gray=False, actions='needed', lives='unused', noops=30, autostart=False, resize='pillow', clip_reward=False)
    episode_return = 0
    terminal = False
    length = 0
    print(env.act_space)
    ...
    with tqdm(total=200000) as pbar:
        for _ in range(100):
            while not terminal:
                ...
                assert obs['is_terminal'] == obs['is_last']
                length += 1
                pbar.update(1)
                video.append(obs['image'])
            log=np.stack(video,axis=0)
            print(log.shape)
            imageio.mimsave('test.gif',log,fps=20)
            video = []
